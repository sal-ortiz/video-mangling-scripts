...
"$BIN_PATH/ffmpeg" -y -hwaccel auto -i "$TMP_FILE" -map_metadata 0 -vcodec libx265 -crf $CRF_VAL -vf scale=-2:$IMG_HEIGHT -c:a copy "$( readlink --canonicalize "$USER_PATH/$OUTP_FILE" )"
